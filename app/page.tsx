        ...
        <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setDrawer(null); }}>
          <aside className={`evidence-drawer ${drawer === "cases" ? "wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="drawer-title">
            ...
            {drawer === "cases" && <div className="drawer-body case-body">
              ...
              <div className="drawer-help-row"><span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="route-key-help">Route key</button><span className="tip-panel" id="route-key-help" role="tooltip"><strong>Answer</strong>: respond from evidence. <strong>Ask</strong>: request missing information. <strong>Block</strong>: refuse a prohibited request. <strong>Escalate</strong>: send the case to a human workflow.</span></span><span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="source-term-help">Retrieved vs cited</button><span className="tip-panel" id="source-term-help" role="tooltip"><strong>Retrieved</strong> means the sources supplied to Gemini. <strong>Cited</strong> means the sources Gemini identified as support for its claims.</span></span></div>
