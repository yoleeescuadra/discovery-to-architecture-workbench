...
type Drawer = "cases" | "evidence" | "method" | "metrics" | null;
      ...
      {drawer && (
        <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setDrawer(null); }}>
          <aside className={`evidence-drawer ${drawer === "cases" ? "wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="drawer-title">
            <div className="drawer-header"><div><span>Decision trace</span><h2 id="drawer-title">{drawer === "cases" ? "15 recorded test cases" : drawer === "evidence" ? "Approved evidence" : drawer === "metrics" ? "How to read the results" : "How the flow works"}</h2></div><button type="button" className="drawer-close" onClick={() => setDrawer(null)} aria-label="Close drawer">×</button></div>
            ...
            {drawer === "cases" && <div className="drawer-body case-body">
              ...
              <details className="drawer-help"><summary>Help with routes and terms</summary><div><p><strong>Answer</strong>: respond from evidence. <strong>Ask</strong>: request missing information. <strong>Block</strong>: refuse a prohibited request. <strong>Escalate</strong>: send the case to a human workflow.</p><p><strong>Retrieved</strong> means supplied to Gemini. <strong>Cited</strong> means Gemini identified the source as support. IDs such as NS-ENT-07 identify fictional approved documents.</p><p><strong>Northstar</strong>: the fictional product. <strong>SSO</strong>: single sign-on. <strong>MFA</strong>: multi-factor authentication. A-104 and B-220 are fictional account IDs. Professional and Enterprise are fictional subscription tiers.</p></div></details>
            ...
            {drawer === "metrics" && <div className="drawer-body metric-explainer"><div className="case-context"><span>Your selected focus</span><b>{designLens.test.focusLabel}</b><small>{designLens.test.suiteContext}</small></div><div className="metric-explainer-list">{designLens.test.signals.map((signal) => <article key={signal.label}><span className={`metric-state ${signal.state}`}>{signal.state === "pass" ? "Passed" : "Review"}</span><h3>{signal.value} · {signal.label}</h3><p>{signal.help}</p></article>)}</div></div>}
