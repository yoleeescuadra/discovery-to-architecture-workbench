        ...
        <div className="stage-frame">
          ...
          {stage === "architecture" && (
            <section className="stage-screen focused-screen architecture-screen" aria-labelledby="architecture-title">
              <div className="stage-heading"><span className="eyebrow">Design consequence · You chose {designLens.label} <span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="architecture-color-key">What do the colors mean?</button><span className="tip-panel color-key" id="architecture-color-key" role="tooltip"><span><i className="key-swatch neutral" />Neutral: regular workflow step</span><span><i className="key-swatch model" />Light teal: Gemini model work</span><span><i className="key-swatch control" />Dark teal: fixed guardrails</span><span><i className="key-swatch selected" />Outline: your selected focus</span></span></span></span><h1 id="architecture-title">Because you chose {designLens.label}: {designLens.headline}</h1><p>The complete system stays in place. {designLens.focusNode} receives extra attention because {designLens.consequence.toLowerCase()}</p></div>
        ...
        <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setDrawer(null); }}>
          <aside className={`evidence-drawer ${drawer === "cases" ? "wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="drawer-title">
            ...
            {drawer === "cases" && <div className="drawer-body case-body">
              <div className="case-context"><span>What is being tested</span><b>For Maya&apos;s fictional software product, Northstar, Gemini drafts a support response. Fixed checks compare its route, evidence use and safety controls with the expected result.</b><small>Gemini 3.5 Flash-Lite · recorded prompt · reviewed scoring rules v3.1</small></div>
              <div className="drawer-help-row"><span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="route-key-help">Route key</button><span className="tip-panel" id="route-key-help" role="tooltip"><strong>Answer</strong>: respond from evidence. <strong>Ask</strong>: request missing information. <strong>Block</strong>: refuse a prohibited request. <strong>Escalate</strong>: send the case to a human workflow.</span></span><span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="source-term-help">Retrieved vs cited</button><span className="tip-panel" id="source-term-help" role="tooltip"><strong>Retrieved</strong> means the sources supplied to Gemini. <strong>Cited</strong> means the sources Gemini identified as support for its claims. IDs such as NS-ENT-07 identify the fictional approved documents.</span></span><span className="info-tip"><button type="button" className="tip-trigger subtle-help" aria-describedby="case-terms-help">Case terms</button><span className="tip-panel" id="case-terms-help" role="tooltip"><strong>Northstar</strong>: the fictional software product. <strong>SSO</strong>: single sign-on. <strong>MFA</strong>: multi-factor authentication. <strong>A-104 and B-220</strong>: fictional account IDs. Professional and Enterprise are fictional subscription tiers.</span></span></div>
